...
export function generateConnectionFragment(
  typeName: string,
  fragment: string
): string | null {
  ...
  if (foundType) {
    let outputFragment = '';
    const containsNode = !!foundType.fields.find(
...
export function generateNormalFragment(
  type: Type,
  connection?: boolean
): string | null {
  let outputFragment = '';
        ...
        if (level < THRESHOLD) {
          ...
          if (allChildrenFragmentable) {
            ...
            type?.fields?.forEach((field) =>
              createFragmentWithDeps(field, level + 1)
            );
            outputFragment += `} `;
          }
