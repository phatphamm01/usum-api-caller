...
export const generateUnionType = (object: Type): string => {
  ...
  return `${
    object.description ? `/** ${object.description} */\n` : ''
  } export type ${prefix ? prefix : ''}${ObjectName}${
    suffix ? suffix : ''
  } = ${object.possibleTypes?.map((type) => type.name).join('|')}`;
};
...
export const generateQGLArg = (field: Arg): string => {
  const { isArray, isArrayRequired, isRequired, typeName } =
    SchemaStore.getFieldProps(field);
  return `${isArray ? '[' : ''}${typeName}${isArrayRequired ? '!' : ''}${
    isArray ? ']' : ''
  }${isRequired ? '!' : ''}`;
};
