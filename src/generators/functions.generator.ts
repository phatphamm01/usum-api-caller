...
import { createReactApolloHook } from './hooks.generator';
...
export function createGraphQLFunction({
  ...
  const argsOptional = hasArgs
    ? SchemaStore.areFieldArgsAllOptional(field)
    : false;
  ...
  if (isScalar) {
    ...
    ${methodName}(): ${
      withArgs ? '' : 'Executable'
    }Query${withArgs}<${returnedTypeDisplay}${
    ...
    ${methodName}(fragment${
      genFragments ? '?' : ''
    }: fields: GenFields<${returnedTypeDisplay}>): ExecutableQuery${withArgs}<${returnedTypeDisplay}${
      hasArgs ? ',' + functionArgsTypeName : ''
    }> {
      ...
      const queryTemplate = ${
        genFragments ? `fragment ? ${Query} : defaultQuery` : Query
      }
...
export function generateFunction({
  ...
  if (
    ...
    innerFragment =
      generateConnectionFragment(typeName, innerFragment) ?? innerFragment;
  }
