...
class SchemaConstructor {
  ...
  public get schemaObjectTypes(): Type[] {
    return this.schema.__schema.types.filter(
      (type) => type.kind === TypeKind.OBJECT
    );
  }
  ...
  public get schemaFunctions(): [Field[], Field[], Field[]] {
    ...
    let listQueries =
      _schema.types.find((type) => type.name === QueryType)?.fields ?? [];
    let listMutations: Field[] = [];
    let listSubscription: Field[] = [];

    if (MutationType) {
      listMutations =
        _schema.types.find((type) => type.name === MutationType)?.fields ?? [];
    }
    if (SubscriptionType) {
      listSubscription =
        _schema.types.find((type) => type.name === SubscriptionType)?.fields ??
        [];
    }
  ...
  public isTypeConnection(typeName: string): boolean {
    ...
    if (type && type.fields) {
      return type.fields
        ...
        .some(({ isConnection }) => isConnection);
    }
    return false;
  }
  ...
  public areFieldArgsAllOptional(field: Field): boolean {
    return field.args
      .map(this.getFieldProps)
      .every(({ isRequired }) => !isRequired);
  }

  public getFunctionFieldArgs(field: Field): {
    GQLVariables: string[];
    GQLArgs: string[];
