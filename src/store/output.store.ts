...
class OutputConstructor {
  private signature = '';
  private loader = ora(
    '🔄 Transpiling GraphQL schema to Typescript interfaces'
  );
  ...
  getRenderedFileString(): string {
    ...
    try {
      ...
      if (codegenFunctions)
        CODEGEN_FUNCTIONS = buildCodeGenFunctions(CodeGenType.METHODS);
      if (codegenReactHooks)
        CODEGEN_REACT_HOOKS = buildCodeGenFunctions(CodeGenType.REACT_HOOKS);
      if (codegenTemplates)
        CODEGEN_TEMPLATES = buildCodeGenFunctions(CodeGenType.TEMPLATE);
